...
Hits every public endpoint against every JPEG in tests/, using the same
multipart upload the Nextcloud app sends. Exits non-zero on any failure.

Adapted from the upstream dlib-based smoke test (matiasdelellis/facerecognition-external-model);
descriptor length is 512 here (InsightFace) rather than 128 (dlib).
...
Usage:
    API_KEY=... python3 tests/api_smoke.py
"""
import json
import mimetypes
import os
...
def check_detect(image):
    ...
    if not expect(
        ...
        return None
    if not expect(
        len(face["descriptor"]) == 512, "descriptor is 512-d"
    ):
        return None
    return {
        "left": face["left"],
...
def check_compute(image, face):
    ...
    return expect(
        isinstance(payload, dict)
        and "face" in payload
        and len(payload["face"].get("descriptor", [])) == 512,
        f"body has face with 512-d descriptor, got {payload!r}",
    )
